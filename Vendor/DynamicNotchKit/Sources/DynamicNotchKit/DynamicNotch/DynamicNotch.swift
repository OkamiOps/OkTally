...
public final class DynamicNotch<Expanded, CompactLeading, CompactTrailing>: ObservableObject, DynamicNotchControllable where Expanded: View, CompactLeading: View, CompactTrailing: View {
    ...
    /// [OkTally patch] Extra height, in points, that the COMPACT panel extends below the
    /// physical notch. The stock panel is exactly as tall as the notch, so any content
    /// meant to be continuous across the panel's bottom (e.g. a progress bar) has its
    /// middle hidden behind the hardware cutout — there are no pixels there. A small
