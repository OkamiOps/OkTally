...
struct NotchView<Expanded, CompactLeading, CompactTrailing>: View where Expanded: View, CompactLeading: View, CompactTrailing: View {
    ...
    /// [OkTally patch] The shelf only exists while compact — expanded already has its own
    /// bottom inset, and the hidden state must collapse back to the bare notch size.
    private var compactShelf: CGFloat {
        dynamicNotch.state == .compact ? dynamicNotch.compactBottomShelf : 0
    }
        ...
        ZStack {
            compactContent()
                ...
                .frame(
                    ...
                    height: ((dynamicNotch.state == .compact && dynamicNotch.isHovering) ? dynamicNotch.menubarHeight : dynamicNotch.notchSize.height) + compactShelf
                )
    ...
    func compactContent() -> some View {
        HStack(spacing: 0) {
            if dynamicNotch.state == .compact, !dynamicNotch.disableCompactLeading {
                dynamicNotch.compactLeadingContent
                    ...
                    // [OkTally patch] The shelf lives inside the bottom inset: content
                    // keeps its vertical position, only the black shape gets taller.
                    .safeAreaInset(edge: .bottom, spacing: 0) { Color.clear.frame(height: 8 + compactShelf) }
            ...
            if dynamicNotch.state == .compact, !dynamicNotch.disableCompactTrailing {
                dynamicNotch.compactTrailingContent
                    ...
                    .safeAreaInset(edge: .top, spacing: 0) { Color.clear.frame(height: 4) }
                    // [OkTally patch] Same shelf as the leading side.
                    .safeAreaInset(edge: .bottom, spacing: 0) { Color.clear.frame(height: 8 + compactShelf) }
        ...
        // [OkTally patch] Without the shelf here, the outer frame would grow but this
        // fixed height would keep the ideal size at the bare notch height, and the whole
        // row would float half a shelf above where it belongs.
        .frame(height: dynamicNotch.notchSize.height + compactShelf)
